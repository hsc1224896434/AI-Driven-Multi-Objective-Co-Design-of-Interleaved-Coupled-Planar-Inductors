...
class Transformer(nn.Module):
    def __init__(self, 
        ...
        #    dim_proj_fusion :int, number of neurons in the linear layer of the fusion projecter 融合投影器线性层的神经元数量
        #    n_encoder_layers: int, number of stacked encoder layers in the encoder 编码器中堆叠的编码器层的数量
        #    n_heads: int, the number of attention heads (aka parallel attention layers) 注意力头的数量（即并行注意力层的数量）
        #    dropout_encoder: float, the dropout rate of the encoder
        #    dropout_pos_enc: float, the dropout rate of the positional encoder
        #    dim_feedforward_encoder: int, number of neurons in the linear layer of the encoder 编码器线性层的神经元数量
        ...
        self.proj_fusion = nn.Sequential(
            nn.Linear(dim_hidden+3, dim_proj_fusion),
            nn.Tanh(),
    ...
    def forward(self, B_curve: Tensor, Temp: Tensor, Freq: Tensor, H_dc: Tensor ) -> Tensor:
        ...
        H_dc = H_dc.unsqueeze(1).repeat(1, len_seq, 1)  # (bs,1)->(bs,1024,1)
        Freq = Freq.unsqueeze(1).repeat(1, len_seq, 1) # (bs,1)->(bs,1024,1)

        feat = self.proj_fusion(torch.cat([B_curve, Temp, Freq, H_dc], dim=2)) # (bs,1024,24+1+1)->(bs,1024,1)
...
class Lit_model(pl.LightningModule):
    ...
    def forward(self, in_B, in_F, in_T, in_H_dc, out_P):
        pred_P = self.net(in_B, in_F, in_T, in_H_dc)
        return pred_P

    def training_step(self, batch, batch_idx):
        in_B, in_F, in_T, in_H_dc, out_P, gt_P = batch

        pred_P = self.net(in_B, in_F, in_T, in_H_dc)

        loss = self.metric(pred_P, out_P)
        self.log("train_loss", loss, prog_bar=True)
        if torch.isnan(pred_P).any() or torch.isinf(pred_P).any():
            print("pred_P contains NaN or Inf in training_step.")
        return loss
